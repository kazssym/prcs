...
/* If this bit is set, do not process the GNU reg2ex2 operators.
   If not set, then the GNU reg2ex2 operators are recognized. */
#define RE_NO_GNU_OPS (RE_NO_POSIX_BACKTRACKING << 1)

/* If this bit is set, turn on internal reg2ex2 debugging.
   If not set, and debugging was on, turn it off.
   This only works if reg2ex2.c is compiled -DDEBUG.
   We define this bit always, so that all that's needed to turn on
   debugging is to recompile reg2ex2.c; the calling code can always have
...
/* If any error codes are removed, changed, or added, update the
   `re_error_msg' table in reg2ex2.c.  */
typedef enum
{
#ifdef _XOPEN_SOURCE
  REG2_ENOSYS = -1,	/* This will never happen for this implementation.  */
#endif
  ...
  REG2_ESUBREG,		/* Invalid back reference.  */
...
typedef struct re_pattern_buffer reg2ex2_t;

...
/* Like `re_search', but return how many characters in STRING the reg2ex2p
...
/* POSIX compatibility.  */
extern int reg2comp _RE_ARGS ((reg2ex2_t *__restrict __preg,
			      const char *__restrict __pattern,
			      int __cflags));

extern int reg2ex2ec _RE_ARGS ((const reg2ex2_t *__restrict __preg,
